...
(function(){
  var lc, ldld, el, dataset, sheets, i, ctrl, vis, cfg, ldrs, ldcp;
  lc = {
    hover: {},
        ...
        action: {
          click: {
            ...
            "config-btn": function(){
              ...
              return view.get('config-panel').classList.toggle('z-float', true);
            },
            "data-btn": function(){
              view.get('config-panel').classList.toggle('invisible', true);
              return view.get('config-panel').classList.toggle('z-float', false);
            },
            int: function(arg$){
              var names;
              names = arg$.names;
              cfg.int = names.filter(function(it){
                return it !== 'int';
              })[0];
  ...
  vis = {
    ...
    set: function(name, value){
      ...
      cfg.pal = pal = value.colors.map(function(it){
        return ldColor.hex(it);
      });
      return vis.render();
    },
    render: function(){
      var d, intAlg, root, fsize, j;
      if (cfg.pal) {
        d = cfg.pal.map(function(d, i){
          return i / (cfg.pal.length - 1);
        });
        d = d.map(function(d, i){
          return d * (cfg.coverage * 2) + (cfg.midPoint - cfg.coverage);
        });
        d = d.map(function(it){
          var ref$;
          return (ref$ = it > 0 ? it : 0) < 1 ? ref$ : 1;
        });
        intAlg = cfg.int === 'hcl'
          ? d3.interpolateHcl
          : cfg.int === 'hsl'
            ? d3.interpolateHsl
            : d3.interpolateRgb;
        lc.scale = d3.scaleLinear().domain(d).range(cfg.pal).interpolate(intAlg);
      }
      ...
      root.select('g').selectAll('circle').attr('fill', function(){
        return ldColor.hex(cfg.bubbleColor);
      ...
      if (d) {
        ...
        root.select('g.legend').selectAll('g').data(d).enter().append('g').each(function(n, i){
          d3.select(this).append('text');
          return d3.select(this).append('rect');
        });
        j = 0;
        root.select('g.legend').selectAll('g').each(function(n, i){
          j = d[i + 1] === n
            ? j
            : j + 1;
          d3.select(this).attr('transform', "translate(" + fsize * 0.75 + " " + (fsize + j * fsize * 1.5) + ")").style('display', d[i + 1] === n ? 'none' : 'block');
          d3.select(this).select('text').attr('x', '1.75em').attr('y', '1em').text(Math.round((n * (lc.max - lc.min) + lc.min) * 100) / 100);
          return d3.select(this).select('rect').attr('x', 0).attr('y', 0).attr('width', '1.5em').attr('height', '1em').attr('fill', lc.scale(n));
        });
      }
  ...
  ctrl.init();
  ldrs = new ldSlider({
    root: ld$.find('#slider-mid-point', 0),
    min: 0,
    max: 100,
    step: 0.1,
    from: 50
  ...
  ldrs = new ldSlider({
    root: ld$.find('#slider-coverage', 0),
    min: 0,
    max: 100,
    step: 0.1,
    from: 50
